...
const MovieDetailPage = () => {
  ...
  const {
    ...
    error: creditsError,
  } = useCustomFetch<{ cast: Cast[]; crew: Crew[] }>(
  ...
  if (movieError || creditsError)
    return (
      <div>
        {movieError || creditsError || "영화 정보를 불러오는 데 실패했습니다."}
      </div>
    );
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div>
            ...
            <p className="text-sm">
              개봉일: <span>{movie.release_date}</span>
            </p>
            <p className="text-sm">
              평점: <span>{movie.vote_average}</span>
            </p>
            <p className="text-sm mb-7">
              상영 시간:<span> {movie.runtime}분</span>
            </p>
            <p className="italic text-sm mb-10 font-extralight ">
              {movie.overview}
            </p>
          </div>
        </div>
