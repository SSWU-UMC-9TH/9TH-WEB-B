...
import { IoEye, IoEyeOff } from "react-icons/io5"; // 👁️ 아이콘 추가
...
const SignupPage = () => {
  ...
  // 각 단계별 유효성 검사 상태
  const emailValid = z.string().email().safeParse(watch("email")).success;
  const passwordValid =
    watch("password").length >= 6 && watch("password") === watch("passwordCheck");
  const nameValid = watch("name").trim().length > 0;
        ...
        {currentStep === 2 && (
          <>
            ...
            <div className="relative w-full">
              <input
                {...register("password")}
                type={showPassword ? "text" : "password"} // 👁️ 토글
                placeholder="비밀번호"
            ...
            <div className="relative w-full">
              <input
                ...
                type={showPasswordCheck ? "text" : "password"} // 👁️ 토글
                placeholder="비밀번호 확인"
                className={`border w-full p-[10px] rounded-sm pr-10 ${
