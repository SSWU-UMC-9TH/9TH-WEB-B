...
interface CustomIntercalAxiosRequestConfig extends InternalAxiosRequestConfig {
...
axiosInstance.interceptors.response.use(
    (response) => response,
    async (error) => {
        const originalRequest: CustomIntercalAxiosRequestConfig = error.config;

        // 401 에러면서 아직 재시도 하지 않은 요청 처리
        if (
            error.response &&
